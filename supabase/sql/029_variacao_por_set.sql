-- Sprint 5 — quanto CADA SET moveu (transparência da regra nº 4)
-- Rode no Supabase: SQL Editor → New query → colar → Run.
--
-- ============================================================
-- POR QUE ISTO É POSSÍVEL, E POR QUE NÃO É RATEIO
-- ============================================================
-- O motor calcula em blocos de um DIA (item 11), e isso não muda: a noite
-- é o evento, e é contra o nível do início do dia que os sets são julgados.
--
-- Mas a variação do dia **se decompõe exatamente** por set. A fórmula do
-- Glicko para o bloco é:
--
--     variação = (q / denominador) × Σ [ g × (resultado − esperado) × peso ]
--
-- O `(q / denominador)` é um só para o dia inteiro; o que muda de um set
-- para outro é apenas o termo dentro do somatório. Então guardar o termo de
-- cada set, multiplicado por esse mesmo fator, dá números que **somam
-- exatamente** a variação do dia.
--
-- Isso importa: não é rateio proporcional nem estimativa. É a mesma conta,
-- aberta. Se fosse aproximação, a soma não fecharia e a tela mentiria — o
-- oposto do que a regra nº 4 pede.
--
-- Foi o que permitiu, em 09/08/2026, mostrar ao fundador que numa noite de
-- três sets o Carlos ganhou +9,4, +10,3 e perdeu −18,8, fechando a noite em
-- +0,9. Aquele detalhamento foi reconstruído por fora; agora fica gravado.

alter table public.rating_bloco_sets
  add column if not exists variacao numeric;
...
-- ============================================================
-- DEPOIS DE RODAR, RECALCULE PARA PREENCHER A COLUNA NOVA
-- ============================================================
--     select public.recalcular_ratings();
--
-- Os ratings NÃO devem mudar — a conta é a mesma, ela só passou a guardar o
-- detalhe. Se algum rating mudar, é erro nesta alteração, não regra nova.
