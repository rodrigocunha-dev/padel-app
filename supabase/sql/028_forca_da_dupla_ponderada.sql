-- Sprint 5 — força da dupla: média ponderada para o elo mais fraco
-- Rode no Supabase: SQL Editor → New query → colar → Run.
--
-- ============================================================
-- POR QUE ESTE PARÂMETRO EXISTE
-- ============================================================
-- O `027` calcula a força de uma dupla pela MÉDIA SIMPLES dos dois. Isso
-- tem um furo que o fundador levantou em 09/08/2026, e que é específico do
-- padel: **todo mundo joga na bola do mais fraco.**
--
-- Pela média simples, um 7ª Fraco (1000) com um 1ª Forte (2000) dá uma
-- dupla de 1500 — "exatamente equilibrada" contra dois 4ª Médio, 50% de
-- chance. Na quadra isso é falso: o elo fraco é atacado o jogo inteiro e a
-- dupla desigual rende bem menos do que a média sugere.
--
-- O parâmetro `peso_do_mais_fraco` permite pender a conta para o elo fraco:
--
--   força da dupla = (mais fraco × peso) + (mais forte × (1 − peso))
--
--   0,5  → média simples (comportamento de hoje)
--   0,6  → dupla 1000+2000 vale 1400 · chance cai de 50% para 36%
--   0,7  → vale 1300 · chance cai para 24%
--
-- ⚠️ **NASCE EM 0,5, ou seja, nada muda agora.** Decisão consciente do
-- fundador: mexer nisso muda TODOS os números do motor, e não temos
-- evidência nenhuma para escolher 0,6 em vez de 0,5 — seria palpite. O
-- parâmetro entra agora porque o custo de deixá-lo pronto é uma linha, e
-- porque quando o beta mostrar como duplas desiguais realmente se
-- comportam, o ajuste vira trocar um número e mandar recalcular.
--
-- Efeito colateral bom, quando for ligado: se a dupla desigual passa a ser
-- considerada mais fraca, vencer com ela vira surpresa maior e **os dois**
-- ganham mais — com o mais fraco ganhando mais ainda, porque costuma ser o
-- mais incerto. É o caminho defensável para a intuição de "o mais fraco
-- deveria ganhar mais", sem cair na armadilha de premiar quem foi carregado.

alter table public.rating_parametros
  add column if not exists peso_do_mais_fraco numeric not null default 0.5
    check (peso_do_mais_fraco >= 0.5 and peso_do_mais_fraco <= 1);
-- O piso de 0,5 é de propósito: abaixo disso a conta penderia para o mais
-- FORTE, que é o oposto do que a tática do jogo indica.
...
-- Com o peso em 0,5 o resultado é idêntico ao do 027 — a média ponderada
-- com pesos iguais É a média simples. Rodar
--     select public.recalcular_ratings();
-- depois deste script deve devolver exatamente os mesmos ratings. Se mudar
-- algum, é sinal de erro nesta troca, não de mudança de regra.
